...
export default function Home() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      if (supabase) {
        ...
        setUser({
          email: 'demo-user@taskautomator.local',
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      let loadedSchedules: ScheduledTask[] = [];
      ...
      try {
        if (supabase) {
          ...
          if (currentUser) {
            ...
            if (!filesError && dbFiles) {
              isSupabaseActive = true;
              if (dbFiles.length > 0) {
                dbFiles.forEach((f: any) => {
                  loadedFiles[f.name] = f.content;
                });
              } else {
                // Seed default files in database
                for (const [name, content] of Object.entries(DEFAULT_FILES)) {
                  await supabase.from('files').insert({ name, content, user_id: currentUserId });
                }
            ...
            if (!schedError && dbSchedules) {
              loadedSchedules = dbSchedules.map((s: any) => ({
                id: s.id,
                name: s.name,
                interval: s.interval,
                code: s.code,
                isActive: s.is_active,
                lastRun: s.last_run ? new Date(s.last_run).toLocaleTimeString() : undefined,
                nextRun: s.next_run ? new Date(s.next_run).toLocaleTimeString() : undefined,
              }));
            }
          }
        }
      ...
      if (loadedFiles[selectedFile]) {
        setEditorContent(loadedFiles[selectedFile]);
      }

      setSchedules(loadedSchedules);
      loadedSchedules.forEach((task) => {
        if (task.isActive) {
      ...
      if (isSupabaseActive) {
        addLog('success', 'Connected to Supabase database successfully.');
      } else {
        addLog('info', 'Loaded workspace state from LocalStorage.');
      }
    };
  ...
  const saveFilesToLocal = async (newFiles: Record<string, string>) => {
    setFiles(newFiles);
    ...
    try {
      if (supabase) {
        ...
        if (userId) {
          await supabase
            .from('files')
            .upsert({ name: selectedFile, content: newFiles[selectedFile], user_id: userId }, { onConflict: 'name,user_id' });
        }
      }
    } catch (e) {
      console.error('Supabase file upsert failed:', e);
    }
  };

  const saveSchedulesToLocal = async (newSchedules: ScheduledTask[]) => {
    ...
    try {
      if (supabase) {
        ...
        const userId = session?.user?.id;
        if (userId) {
          for (const task of newSchedules) {
            await supabase.from('schedules').upsert({
              id: task.id.length > 8 ? task.id : undefined, // Ensure it's UUID or let DB generate
              name: task.name,
              interval: task.interval,
              code: task.code,
              is_active: task.isActive,
              last_run: task.lastRun ? new Date().toISOString() : null,
              user_id: userId,
            });
          }
        }
      ...
      console.error('Supabase schedules upsert failed:', e);
    }
  };

  // Add a log message
  const addLog = async (type: LogMessage['type'], message: string) => {
    ...
    try {
      if (supabase) {
        ...
        const userId = session?.user?.id;
        if (userId) {
          await supabase.from('logs').insert({ type, message, user_id: userId });
        }
      }
    } catch (e) {
      console.error('Supabase log insert failed:', e);
    }
  };
  ...
  const startTaskInterval = (task: ScheduledTask) => {
    ...
    const intervalId = setInterval(() => {
      const interpreter = new DSLInterpreter(
        files,
        (logMsg) => addLog(logMsg.type, logMsg.message),
        (updatedFiles) => saveFilesToLocal(updatedFiles),
        () => {} // No nested schedules
      );
      
      addLog('info', `[Trigger] Running scheduled task: "${task.name}"`);
  ...
  const handleCreateFile = async () => {
    ...
    setFiles(updated);
    localStorage.setItem('lscript_files', JSON.stringify(updated));
    ...
    try {
      if (supabase) {
        ...
        const userId = session?.user?.id;
        if (userId) {
          await supabase.from('files').insert({ name, content: '# New Automation script\n', user_id: userId });
        }
      }
    } catch (e) {
      console.error('Supabase file insert failed:', e);
    }
  ...
  const handleDeleteFile = async (fileNameToDelete: string) => {
    ...
    try {
      if (supabase) {
        ...
        const userId = session?.user?.id;
        if (userId) {
          await supabase.from('files').delete().eq('name', fileNameToDelete).eq('user_id', userId);
        }
      }
    } catch (e) {
      console.error('Supabase file delete failed:', e);
    }
      ...
      <header className="h-16 border-b border-white/10 glass-panel flex items-center justify-between px-md sm:px-lg z-10 flex-shrink-0">
        <div className="flex items-center gap-sm sm:gap-md">
          ...
          <button 
            ...
            className="p-1.5 hover:bg-white/5 rounded-full text-on-surface-variant hover:text-white transition-colors cursor-pointer"
          ...
          <div className="p-md border-b border-white/5 flex justify-between items-center flex-shrink-0">
            ...
            <div className="flex items-center gap-xs">
              ...
              <button 
                ...
                className="p-1 hover:bg-white/5 rounded-full text-on-surface-variant hover:text-white transition-colors cursor-pointer"
                ...
                <div className="w-full md:w-80 h-48 md:h-auto flex flex-col bg-surface-container-lowest/20 overflow-hidden select-none border-t md:border-t-0 md:border-l border-white/5">
                  ...
                  <div className={`flex flex-col overflow-hidden border-b border-white/5 transition-all duration-300 ${showTokens ? 'flex-1' : 'h-11'}`}>
                    <div 
                      ...
                      className="p-md bg-surface-container/50 flex justify-between items-center cursor-pointer hover:bg-white/5 transition-colors"
                    >
                      <span className="font-display font-semibold text-[10px] tracking-wider text-on-surface-variant uppercase">Token Highlights</span>
                      <span className="text-[10px] text-on-surface-variant">{showTokens ? '▼' : '▶'}</span>
                    </div>
                    {showTokens && (
                  ...
                  <div className={`flex flex-col overflow-hidden bg-black/10 transition-all duration-300 ${showCheatSheet ? 'h-72 md:h-80' : 'h-11'}`}>
                    ...
                    >
                      <span className="font-display font-semibold text-[10px] tracking-wider text-on-surface-variant uppercase">DSL Syntax Reference</span>
                      <span className="text-[10px] text-on-surface-variant">{showCheatSheet ? '▼' : '▶'}</span>
                    </div>
                    {showCheatSheet && (
                      <div className="flex-1 p-md overflow-y-auto text-[11px] text-on-surface-variant space-y-3 select-text font-sans">
                        <div>
                          <span className="font-mono text-primary-fixed font-semibold block">create file &quot;name&quot; with content &quot;data&quot;</span>
                          <p className="text-[10px] text-outline mt-0.5">Creates a mock file in workspace.</p>
                        </div>
                        <div>
                          <span className="font-mono text-primary-fixed font-semibold block">append file &quot;name&quot; with content &quot;data&quot;</span>
                          <p className="text-[10px] text-outline mt-0.5">Appends content to a file.</p>
                        </div>
                        <div>
                          <span className="font-mono text-primary-fixed font-semibold block">read file &quot;name&quot;</span>
                          <p className="text-[10px] text-outline mt-0.5">Reads and logs file content.</p>
                        </div>
                        <div>
                          <span className="font-mono text-primary-fixed font-semibold block">delete file &quot;name&quot;</span>
                          <p className="text-[10px] text-outline mt-0.5">Deletes a file from workspace.</p>
                        </div>
                        <div>
                          <span className="font-mono text-primary-fixed font-semibold block">http get &quot;url&quot;</span>
                          <p className="text-[10px] text-outline mt-0.5">Sends HTTP request &amp; logs body.</p>
                        </div>
                        <div>
                          <span className="font-mono text-primary-fixed font-semibold block">every X seconds do &#123; ... &#125;</span>
                          <p className="text-[10px] text-outline mt-0.5">Schedules repeating block loops.</p>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <div className="flex-1 p-lg overflow-y-auto flex flex-col gap-lg">
                ...
                {schedules.length === 0 ? (
                  <div className="flex-1 border border-dashed border-white/10 rounded-xl flex flex-col items-center justify-center p-xl text-center">
                    ...
                    <h3 className="w-full text-on-surface font-semibold mb-xs text-center">No active scheduled loops</h3>
                    <p className="w-full text-sm text-on-surface-variant/80 max-w-xs text-center mx-auto mt-2 leading-relaxed">Use the syntax &quot;every X seconds do &#123; ... &#125;&quot; in your script to spin up scheduled runner tasks.</p>
                  </div>
                ) : (
            ...
            {activeTab === 'settings' && (
              <div className="flex-1 p-lg overflow-y-auto flex flex-col gap-lg">
                <div>
                  ...
                  <p className="text-body-sm text-on-surface-variant">Modify active background overlays, animations, virtual FS metrics, and view profile settings.</p>
                </div>

                {/* User Profile Info Card */}
                <div className="glass-panel p-md rounded-xl space-y-sm border border-white/5">
                  <h3 className="font-display font-semibold text-body-md text-white">User Profile</h3>
                  {user ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-md text-xs">
                      <div className="flex flex-col gap-xs">
                        <span className="text-outline uppercase tracking-wider text-[10px]">Email Address</span>
                        <span className="text-white font-mono text-sm">{user.email}</span>
                      </div>
                      <div className="flex flex-col gap-xs">
                        <span className="text-outline uppercase tracking-wider text-[10px]">Date Joined</span>
                    ...
                    <p className="text-xs text-outline italic">Loading user profile details...</p>
                  )}
                </div>
